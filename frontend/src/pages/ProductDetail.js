  ...
  if (loading) {
    return (
      <div className="min-h-screen pt-20 flex items-center justify-center">
        <div className="w-12 h-12 border-2 border-[#CCFF00] border-t-transparent animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen pt-20 md:pt-24" data-testid="product-detail-page">
      ...
      <div className="max-w-[1600px] mx-auto px-4 md:px-8 py-4">
        <Link
          to="/catalog"
          className="inline-flex items-center gap-2 text-sm font-mono uppercase tracking-widest text-white/50 hover:text-white transition-colors duration-300"
      ...
      <div className="max-w-[1600px] mx-auto px-4 md:px-8 pb-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
          ...
          >
            {/* Main Image */}
            <div className="aspect-[4/5] bg-[#0a0a0a] overflow-hidden" data-testid="main-image">
              {product.images?.[activeImage] ? (
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <span className="text-white/20 font-mono">No Image</span>
                </div>
              )}
            </div>
                ...
                {product.images.map((img, index) => (
                  <button
                    ...
                    className={`w-20 h-20 flex-shrink-0 overflow-hidden border-2 transition-colors duration-300 ${
                      activeImage === index ? 'border-[#CCFF00]' : 'border-transparent'
                    }`}
          ...
          >
            ...
            {/* Name */}
            <h1 className="font-heading text-4xl md:text-5xl lg:text-6xl leading-tight" data-testid="product-name">
              {name}
            </h1>

            {/* Price */}
            <p className="font-mono text-lg text-white/70" data-testid="product-price">
            ...
            {description && (
              <div className="pt-4 border-t border-white/10">
                <h3 className="font-mono text-xs uppercase tracking-widest text-white/50 mb-3">
                  {t('product.description')}
                </h3>
                <p className="text-white/70 leading-relaxed" data-testid="product-description">
                  {description}
                </p>
              </div>
            )}
                ...
                <div className="flex flex-wrap gap-2" data-testid="size-selector">
                  {product.sizes.map((size) => (
                    <button
                      ...
                      className={`min-w-[48px] h-12 px-4 font-mono text-sm uppercase border transition-all duration-300 ${
                        selectedSize === size
            ...
            <div className="pt-6">
              <Button
                ...
                className={`w-full h-14 font-bold text-sm uppercase tracking-wider transition-all duration-300 ${
                  addedToCart
